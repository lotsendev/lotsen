...
func NewAccessLogBuffer(capacity int) *AccessLogBuffer {
	...
	return &AccessLogBuffer{entries: make([]AccessLogEvent, capacity)}
}

// Log appends a new access log entry.
func (b *AccessLogBuffer) Log(entry AccessLogEvent) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *AccessLogBuffer) List(limit int) []AccessLogEvent {
	b.mu.RLock()
	defer b.mu.RUnlock()
	...
	out := make([]AccessLogEvent, 0, limit)
	for i := 0; i < limit; i++ {
